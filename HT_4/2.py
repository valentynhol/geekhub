...
ці гроші додаються до суми вкладу і 
в наступному році на них також нараховуються відсотки). 
Параметр < percents > є необов'язковим і 
має значення по замовчуванню < 10 > (10%). 
Функція повинна принтануть і вернуть суму, яка буде на рахунку.
"""

def bank(a, years, percents = 10, *args):
...
percents = input("Під який відсоток?(не обов'язково) ")

if percents:
    print(bank(a, years, int(percents))
else:
    print(bank(a, years))